...
function setupEmoji(){const items=['☺','◡̈','☼','✦','◇','♨','☕','⚡','♥','◉','✈','♟'];[['#taskEmoji','☺'],['#rewardEmoji','✦']].forEach(([selector,initial])=>{const box=$(selector);items.forEach(icon=>{const b=document.createElement('button');b.type='button';b.textContent=icon;b.classList.toggle('active',icon===initial);b.onclick=()=>{$$('button',box).forEach(x=>x.classList.remove('active'));b.classList.add('active');box.nextElementSibling.value=icon};box.append(b)})})}
function setupForms(){
