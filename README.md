...
Під час кожного запуску сервер автоматично реєструє Telegram webhook, команду `/start` і кнопку меню Mini App. Адреса береться з Railway-змінної `RAILWAY_PUBLIC_DOMAIN`; за потреби її можна перевизначити через `APP_URL=https://your-domain.example`.
